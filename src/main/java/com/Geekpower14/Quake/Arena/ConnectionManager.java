...
import com.Geekpower14.Quake.Listener.SocketListener;
...
public class ConnectionManager {
	...
	public ConnectionManager(Quake pl)
	{
		plugin = pl;
	...
	public boolean onPlayerConnect(Player p)
	{
		...
		{
			p.sendMessage(ChatColor.RED + "Une erreur s'est produite. Aucune arène ne vous a été assignée.");
	...
	{
		thread = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, new Runnable()
        {
	          public void run() {
	...
	public void sendArenasInfosToServ(String ip, int port, boolean first)
	{
		...
		try{
			...
			if(first)
			{
				...
				out.println("Infos" + ":" + plugin.BungeeName + ":" + Bukkit.getIp() + ":" + plugin.DefaultPort + ":");
				out.flush();
			}else
			{
				//0 : Type ID du serv
			...
			for(Arena arena : plugin.am.getArenas())
			{
				...
				out.println("Arena" 
						+ ":" + arena.getUUID().toString()
						+ ":" + arena.name 
						+ ":" + arena.getActualPlayers() 
						+ ":" + arena.getMaxPlayers() 
						+ ":" + arena.Map_name 
						+ ":" + arena.eta.getString() 
